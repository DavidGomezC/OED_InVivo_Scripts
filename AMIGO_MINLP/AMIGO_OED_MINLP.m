...
function [results]=AMIGO_OED_MINLP(input_file,run_ident,opt_solver);
...
%    
    ...
    [inputs_def]= AMIGO_private_defaults_MINLP;
   
    [inputs_def]= AMIGO_public_defaults_MINLP(inputs_def);
...
%
    ...
    [inputs,results]=AMIGO_check_model_MINLP(input_file,inputs_def); %,results_def 
...
%
    ...
    % simulations during the optimization
     ...
     [results,privstruct]=AMIGO_call_OPTsolver_MINLP('OED',inputs.minlpsol.minlpsolver,inputs.OEDsol.voed_guess,inputs.OEDsol.voed_min,inputs.OEDsol.voed_max,inputs,results,privstruct);      
      
     %%%%%%%%%%% STOPED HERE %%%%%%%%%%
     
     
     privstruct.oed=results.minlpsol.vbest;
     [privstruct,inputs]=AMIGO_transform_oed(inputs,results,privstruct);  
     ...
     [results,privstruct]= AMIGO_post_report_OED_MINLP(inputs,results,privstruct);
