function [inputs] = SetAMIGOStructureOED_WithLocalSolver(tvec, switchT, inpbounds, Identifier)
    ...
    %% Path details
    results_folder = strcat('DISCOED2',char(Identifier),'_',datestr(now,'yyyy-mm-dd'));
    ...
    inputs.exps.u_min{1}=round(inpbounds(1,:));    % Lower boundary for the input value
    inputs.exps.u_max{1}=round(inpbounds(2,:)); % Upper boundary for the input value
%     inputs.exps.noise_type='hetero';           % Experimental noise type: Homoscedastic: 'homo'|'homo_var'(default)
    ...
    % OPTIMIZATION
%     %oidDuration=600;
    inputs.minlpsol.minlpsolver='nomad';
