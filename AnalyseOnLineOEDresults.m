...
function [] = AnalyseOnLineOEDresults(identif)

    resu = load(strcat(['ResultsOnLineOED/OLOEDres_',identif,'.mat']));
    
    if isfile(strcat(['ResultsOnLineOED/OLOEDreslocal_',identif,'.mat']))
        resuLoc = load(strcat(['ResultsOnLineOED/OLOEDreslocal_',identif,'.mat']));
    else
        resuLoc = [];
    end
    ...
    if ~isfolder("ResultsOnLineOED\Plots")
        mkdir("ResultsOnLineOED\Plots")
    end
    ...
    saveas(cc,strjoin(["ResultsOnLineOED\Plots\OLOED_ConvergencePlot_GlobalSolver_", identif,".png"],""))
    ...
    if ~isempty(resuLoc)
        ...
        saveas(cc,strjoin(["ResultsOnLineOED\Plots\OLOED_ConvergencePlot_LocalSolver_", identif,".png"],""))
    end
