...
function Results_multistart = ssm_multistart_MINLP(problem,opts,varargin)
...
default      = ssm_defaults_MINLP2;
opts         = ssm_optset_MINLP2(default,opts);
weight       = opts.weight;
tolc         = opts.tolc;
...
% Load aux files:
if isFjacDefined
    ssm_aux_local_MINLP(problem.f,problem.fjac,x_L,x_U,c_L,c_U,neq,local_solver,nvar,varargin{:});
else
    ssm_aux_local_MINLP(problem.f,[],x_L,x_U,c_L,c_U,neq,local_solver,nvar,varargin{:});
end
...
multix = [x_0; multix];
if (int_var) || (bin_var)
    multix = ssm_round_int_MINLP(multix,int_var+bin_var,x_L,x_U);
end
nrun     = size(multix,1);
f0       = zeros(nrun,1);
tic
...
for ind_search = 1:nrun
    ...
    if strcmp(local_solver,'mix')
        [this_local_solver,obj_fun] = ssm_mix_MINLP(ind_search,nrun);
        this_f                      = obj_fun;
        ...
        if isFjacDefined
            ssm_aux_local_MINLP(this_f,problem.fjac,x_L,x_U,c_L,c_U,neq,this_local_solver,nvar,varargin{:});
        else
            ssm_aux_local_MINLP(this_f,[],x_L,x_U,c_L,c_U,neq,this_local_solver,nvar,varargin{:});
        end
        %-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
...
%%% AFV start handling errors: %%%
  try
    if isFjacDefined
        [x,fval,exitflag,numeval]=ssm_localsolver_MINLP(x0,x_L,x_U,c_L,c_U,neq,ndata,int_var,bin_var,fobj,fjac,...
            this_local_solver,opts.local.iterprint,opts.local.tol,weight,nconst,tolc,opts.local,varargin{:});
    else
        [x,fval,exitflag,numeval]=ssm_localsolver_MINLP(x0,x_L,x_U,c_L,c_U,neq,ndata,int_var,bin_var,fobj,[],...
            this_local_solver,opts.local.iterprint,opts.local.tol,weight,nconst,tolc,opts.local,varargin{:});
    end
  catch
