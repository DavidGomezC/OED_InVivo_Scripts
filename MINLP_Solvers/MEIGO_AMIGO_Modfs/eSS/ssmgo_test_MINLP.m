...
function ssmgo_test_MINLP(solver,nproblem,noptim,pnames,lb,ub,prob,options,test,param);
    ...
    for j=1:noptim
        ...
        if isempty(param) | isempty(param{nfunc})
            if strmatch(lower(solver),'ssm')
                ...
                Results=ess_kernel_MINLP(problem,opts);
            end
            ...
            if strmatch(lower(solver),'ssm')
                ...
                Results=ess_kernel_MINLP(problem,opts,param{nfunc}{:});
            end
        end
