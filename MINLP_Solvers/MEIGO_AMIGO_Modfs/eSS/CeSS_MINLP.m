function Results = CeSS_MINLP(problem,opts)

    %--- Read options, initialize variables: ------------------------------
    par_struct = get_CeSS_options_MINLP(problem,opts);
	...
	for iteration = 1:opts.n_iter   		
		...
		if(opts.is_parallel) % With parallel computing:
			...
			parfor thread = 1:n_threads
                results_iter(thread) = CeSS_thread_MINLP(par_struct,thread);
            end  
            delete(myPOOL)
            %-- With JPAR: ------------------------------------------------
			...
			for thread = 1:n_threads			
				res = ess_kernel_MINLP(par_struct(thread).problem, par_struct(thread).opts);
