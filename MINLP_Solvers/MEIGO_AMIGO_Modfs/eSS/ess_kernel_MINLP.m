...
function [Results]=ess_kernel_MINLP(problem,opts,varargin)
...
for i=1:ndiverse+5+temp
    [val,val_penalty,pena,nlc,include,x] = ssm_evalfc_MINLP2(solutions(i,:),x_L,x_U,fobj,nconst,c_L,c_U,...
...
while (not(fin))
    ...
    while ~continuar
        ...
        if ~isempty(BBB)
            ...
            while ~include
                ...
                [val,val_penalty,pena,nlc,include,x] = ssm_evalfc_MINLP2(new_refset_member,x_L,x_U,fobj,nconst,c_L,c_U,...
                    tolc,weight,int_var,bin_var,varargin{:});
                nfuneval=nfuneval+1;
                if include
    ...
    for i=1:MaxSubSet2
        %Evaluate
        [val,val_penalty,pena,nlc,include,x] = ssm_evalfc_MINLP2(new_comb(i,:),x_L,x_U,fobj,nconst,c_L,c_U,...
            tolc,weight,int_var,bin_var,varargin{:});
        nfuneval=nfuneval+1;
    ...
    for i=1:length(members_to_update);
        [vector,vector_value,vector_penalty,vector_value_penalty,vector_nlc,new_child,new_child_value,...
            new_child_penalty,new_child_value_penalty,new_child_nlc,nfuneval]=ssm_beyond_MINLP(Refset(members_to_update(i),:),...
    ...
    if ~isempty(fff)
		...
		%            newsol=rand(1,nvar).*(xu_log-xl_log)+xl_log;
            ...
            [val,val_penalty,pena,nlc,include,x] = ssm_evalfc_MINLP2(newsol,x_L,x_U,fobj,nconst,c_L,c_U,...
                tolc,weight,int_var,bin_var,varargin{:});
            nfuneval=nfuneval+1;
            if include
    ...
    if fin
        ...
        if local_finish & fin<3
            ...
            if isempty(strmatch(local_solver,local_finish))
                ...
                if isFjacDefined
                    ssm_aux_local_MINLP(problem.f,problem.fjac,x_L,x_U,c_L,c_U,neq,local_finish,nvar,varargin{:});
                else
                    ssm_aux_local_MINLP(problem.f,[],x_L,x_U,c_L,c_U,neq,local_finish,nvar,varargin{:});
                end
            end
            local_tol=3;
            ...
            ssm_delete_files_MINLP(local_finish,c_U);
        end
